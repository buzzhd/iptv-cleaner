...
import time
import gzip # Added to handle EPG decompression
...
# Use the best available EPG sources
EPG_SOURCES = {
    "guide_us": "https://raw.githubusercontent.com/acidjesuz/EPGTalk/master/US_guide.xml.gz",
...
def fetch_epg(name, url):
    ...
    try:
        with requests.get(url, timeout=20) as response:
            ...
            # If the source is compressed, decompress it before saving
            if url.endswith('.gz'):
